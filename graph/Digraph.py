...
class Digraph(AbstractGraph):
    ...
    def __init__(self, input_number_vertexes, input_edges):
        ...
        :param input_is_direcionado:
            :param is_pseudo:
        """

        super().__init__(input_number_vertexes, input_edges, True)
        # self.checkHandshake()
    ...
    def neighbours(self, vertex):
        ...
        Função
        :param: target vertex
        :return: grau de um vertice de um grafo direcionado
        """

        self.testParameters(vertex)

        # Representa a soma dos graus internos e externos
        conected_vertex = set()
        i = 0

        # Percorre as colunas da matriz de pesos
        while i < len(self._matrix):
            # Se houver algum peso em dada posicao, i+1 indica um vizinho de vertex
            if self._matrix[vertex][i] is not None:
                conected_vertex.add(i)
            elif self._matrix[i][vertex] is not None:
                conected_vertex.add(i)
            i += 1

        return conected_vertex
    ...
    def writePrim(self, entrance=None):
        utilsWrite(True, self.runPrim(entrance), "prim.png")
        ...
        def recursive(target):
            ...
            for neighbour in self.neighbours(target):
                if not visited_vertexes[neighbour]:
                    ...
                    if isFather(target, neighbour):
                        foward_edges.append((target, neighbour))
                    else:
